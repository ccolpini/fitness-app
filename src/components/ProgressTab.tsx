    ...
    <div>
      ...
      <div className="flex items-center gap-1 mt-1">
        <input
          ...
          className="pill-input w-full h-10 text-center text-white text-sm font-medium"
          style={{ borderColor: form[key] ? color + '40' : undefined }}
        />
        <span className="text-[10px] w-6" style={{ color: '#666' }}>{unit}</span>
      </div>
    </div>
  );

  return (
    <div className="rounded-xl p-4 mb-5" style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 2px 12px rgba(0,0,0,0.4)' }}>
          ...
          {[1,2,3,4,5].map(v => (
            <button
              ...
              style={form.energy === v ? {
                ...
                color: '#0A0A0A',
              } : { backgroundColor: 'rgba(255,255,255,0.04)', color: '#444' }}
            >{v}</button>
          ))}
        </div>
      </div>
      ...
      <div className="mb-3">
        <label className="section-label">HIP PAIN (0-10)</label>
      ...
      <div className="mb-3">
        <label className="section-label">PR THIS WEEK</label>
        <input
          value={form.pr}
          onChange={e => setForm(f => ({ ...f, pr: e.target.value }))}
          placeholder="e.g. Bench 80kg × 5"
          className="pill-input w-full h-10 mt-1 text-white text-sm px-3"
        />
      </div>
      ...
      <div className="mb-4">
        ...
        <textarea
          ...
          className="pill-input w-full h-16 mt-1 text-white text-sm p-3 resize-none"
        />
      </div>
...
export function ProgressTab() {
  ...
  return (
    <div className="pb-4">
      {/* Header */}
      <div className="relative overflow-hidden px-4 pt-6 pb-4" style={{ background: 'radial-gradient(ellipse at top left, #1a0a15 0%, #0A0A0A 60%)' }}>
        <div className="absolute inset-0 opacity-25"
          style={{ background: 'radial-gradient(ellipse at top right, rgba(255,45,120,0.2), transparent 60%)' }} />
        <div className="relative">
          <h1 className="text-white leading-none mb-1" style={{ fontSize: '36px', fontWeight: 800 }}>PROGRESS</h1>
          <p style={{ fontSize: '14px', color: '#666' }}>Track your recomposition journey</p>
        </div>
      </div>
      ...
      <div className="px-4">
        ...
        {weightChange && (
          <div className="flex gap-2 mb-5 animate-slide-up" style={{ animationDelay: '100ms' }}>
            <div className="rounded-xl flex-1 p-3 flex items-center gap-2" style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 2px 12px rgba(0,0,0,0.4)' }}>
              ...
              <div>
                <p className="text-[10px]" style={{ color: '#666' }}>Weight Δ</p>
            ...
            <div className="rounded-xl flex-1 p-3 flex items-center gap-2" style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 2px 12px rgba(0,0,0,0.4)' }}>
              ...
              <div>
                <p className="text-[10px]" style={{ color: '#666' }}>Waist Δ</p>
        ...
        {/* Chart Toggle */}
        <div className="flex gap-1 p-1 rounded-2xl mb-4" style={{ backgroundColor: '#1E1E1E' }}>
          {[
        ...
        {chartType !== 'strength' && chartData.length > 0 && (
          <div className="rounded-xl p-4 mb-4 animate-fade-in" style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 2px 12px rgba(0,0,0,0.4)' }}>
            ...
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={chartData}>
                ...
                <YAxis tick={{ fontSize: 10, fill: '#444' }} axisLine={false} tickLine={false} domain={['dataMin - 1', 'dataMax + 1']} />
                ...
                <div key={exId} className="mb-4 last:mb-0">
                  ...
                  <ResponsiveContainer width="100%" height={80}>
                    <AreaChart data={chartD}>
                      ...
                      <XAxis dataKey="week" tick={{ fontSize: 8, fill: '#444' }} axisLine={false} tickLine={false} />
        ...
        {chartData.length === 0 && chartType !== 'strength' && (
          <div className="rounded-xl text-center py-8 mb-4" style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 2px 12px rgba(0,0,0,0.4)' }}>
            <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3" style={{ backgroundColor: '#1E1E1E' }}>
            ...
            <p className="text-sm font-medium" style={{ color: '#666' }}>No data yet</p>
            <p className="text-xs mt-1" style={{ color: '#666' }}>Complete your first weekly check-in above</p>
          </div>
        )}

        {chartType === 'strength' && topExercises.length === 0 && (
          <div className="rounded-xl text-center py-8 mb-4" style={{ background: '#141414', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 2px 12px rgba(0,0,0,0.4)' }}>
            <p className="text-sm font-medium" style={{ color: '#666' }}>No workout data yet</p>
            <p className="text-xs mt-1" style={{ color: '#666' }}>Log weights in your sessions to track strength</p>
          </div>
        )}
        ...
        <div className="rounded-xl p-4" style={{ background: '#141414', border: '1px solid rgba(0,212,255,0.1)', boxShadow: '0 2px 12px rgba(0,0,0,0.4)' }}>
